...
-- TCL(Transaction Control language) : 트랜잭션 제어 언어 /  DML에서 수행한 내용을 COMMIT, ROLLBACK 하는 언어

-- DDL(Data Definition Language) : 데이터 정의 언어
-- 객체(OBJECT)를 만들고(CREATE), 수정(ALTER), 삭제(DROP) 등
-- 데이터의 전체구조를 정의하는 언어로 주로 DB관리자, 설계자가 사용함
...
-- 테이브로 생성된 객체는 DROP 구문을 통해 제거할 수 있음
 ...
 *  --> VARCHAR2(10) 컬럼에 'ABC 3BYTE 문자열만 저장하면 나머지 7BYTE를 반환함
 * 
 * DATE : 날짜 타입
 * 
 * BLOB : 대용랑 이진 데이터(4GB)
...
CREATE TABLE MEMBER(
	...
	-- 컬럼에 값이 안들어가면 SYSDATE가 들어가게 하겠다
);
...
-- 전체 컬럼 작성시는 DEFAULT 써야함, 이거는 지정이니까 안적으면 자동 DEFAULT
...
CREATE TABLE USER_USED_UK2(
	...
	CONSTRAINT USER_ID_NAME_U UNIQUE(USER_ID, USER_NAME) -- 둘 다 같으면 안된다
);
...
INSERT INTO USER_USED_PK 
VALUES(1, 'user02', 'pass02', '고길순', '여', '010-4566-5678', 'test@kh.or.kr');
...
-- 컬럼레벨일 경우
-- 컬렴명 자료형(크기) [CONSTRAINT 이름] REFERENCES 참조할테이블명 [(참조할컬럼)] [삭제롤]

-- 테이블레벨일 경우
-- [CONSTRAINT 이름] FOREIGN KEY(적용할컬럼명) REFERENCES 참조할테이블명 [(참조할컬럼)] [삭제롤]
...
CREATE TABLE USER_USED_FK(
	...
	-- 컬럼명 미작성시 USER_GRADE 테이블의 PK를 자동 참조 맨뒤에 (GRADE_CODE)가 생략
);
...
-- FOREIGN KEY로 지저된 컬럼에서 사용되고 있는 값일 경우
-- 제공하는 컬럼의 값은 삭제하지 못함
...
-- GRADE_COD 중 20은 외래키로 참조되고 있지 않으므로 삭제 가능함
...
-- CHECK 제약 조건으로 GENDER 컬럼을 설정했기 때문에 , 남 또는 여 만 기록 가능한데
...
STATUS VARCHAR2(3) DEFAULT 'N' CONSTRAINT NN_STATUS NOT NULL, -- 까지 컬럼레벨 여기선 제약조건 2개불가
...
-- 제약조건, 코멘트는 복사되지 않기 때문에 별도 추가작업이 필요함
...
ALTER TABLE EMPLOYEE_COPY ADD CONSTRAINT EMP_DEPT_CODE 
...
ALTER TABLE EMPLOYEE_COPY ADD CONSTRAINT EMP_JOB_CODE 
...
ALTER TABLE EMPLOYEE_COPY ADD CONSTRAINT EMP_SAL_LEVEL
