...
-- INSTR(컬럼명 | 문자열, '찾을 문자열' [,찾기시작할 위치[, 순번])
...
FROM EMPLOYEE; -- 잘라낼 길이가 INSTR(EMAIL, '@') -1
...
SELECT ROUND(123.456) FROM DUAL; -- 소수점위치를 지정하지 않으면 소수점 첫번째 자리에서 반올림

SELECT ROUND(123.456, 1) FROM DUAL; -- 소수점 두번째 자리에서 반올림
                                    --> 두번째 자리에서 반올림해서 소수점 한자리까지만 표현

SELECT ROUND(123.456 , 0) FROM DUAL; -- 소수점 첫번째 자리에서 반올림(기본값)
...
SELECT TRUNC(123.456) FROM DUAL; -- 소수점 아래를 버리는게 기본
...
SELECT LAST_DAY('2023-02-01') FROM DUAL; -- SYSDATE가 아니고 날짜를 직접넣는것도 가능
...
-- 0 : 숫자 한칸을 의미, 여러개 작성 시 오른쪽 정렬 + 빈칸에 0을 추가해줌
...
SELECT SYSDATE FROM DUAL; -- 2023-01-10 10:21:32.000
...
SELECT TO_DATE('510505', 'RRMMDD') FROM DUAL; -- 1950-05-05
...
-- EMPLOYEE 테이블에서 각 직원이 태어난 생년월일(1990년 5월 13일) 조회
...
SELECT EMP_NAME,
 ...
 /* 숫자 형변환 */
 -- TO_NUMBER(문자데이터,  [포맷]) : 문자형데이터를 숫자 데이터로 변겅
...
-- EMPLOYEE 테이블에서 보너스를 받으면 '0', 안받으면 'X' 조회
...
-- AVG(숫자가 기록된 컬럼명) : 평균
-- 전직원 급여 평균
SELECT ROUND (AVG(SALARY)) FROM EMPLOYEE; -- ROUND 반올림 처리로 소수점 보기좋게

-- 부서 코드가 'D9'인 사원들의 급여 합, 평균
SELECT SUM(SALARY), ROUND(AVG(SALARY)) -- 3번
FROM EMPLOYEE -- 1번
WHERE DEPT_CODE = 'D9'; -- 2번
-- 해석 순서때문에 복잡하게 안하고 이렇게 해도 가능!, 해석순서 중요
...
-- 급여 최대값, 가장 늦은 입사일, 알파벳 순서가 가장느린 이메일
...
FROM EMPLOYEE
WHERE SALARY = (SELECT MAX(SALARY) FROM EMPLOYEE); -- ()안에서 조회한값을 WHERE절의 SALARY와 비교
			-- 서브쿼리 + 그룹함수
...
-- COUNT(컬럼명) : NILL을 제외한 실제 값이 기록된 행 개수를 리턴
