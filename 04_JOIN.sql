...
----------------------------------------------------------------------------------------------------------------
포괄 조인 (FULL은 안됨)		                 왼쪽 외부 조인(LEFT OUTER), 오른쪽 외부 조인(RIGHT OUTER)
                                            + 전체 외부 조인(FULL OUTER, 오라클 구문으로는 사용 못함)
----------------------------------------------------------------------------------------------------------------
자체 조인, 비등가 조인   	                		    JOIN ON
----------------------------------------------------------------------------------------------------------------
카테시안(카티션) 곱		              			 교차 조인(CROSS JOIN),잘못된 테이블 조인 확인용도
CARTESIAN PRODUCT
...
FROM EMPLOYEE
JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID); -- 등가조인(이너조인)


-- 1. 내부 조인(INNER JOIN) ( == 등가 조인(EQUAL JOIN))
--> 연결되는 컬럼의 값이 일치하는 행들만 조인됨.  (== 일치하는 값이 없는(NULL) 행은 조인에서 제외됨. )
...
-- ANSI 표준
-- 사원의 이름과 부서명 조회(DEPT_CODE가 NULL인애들도 뽑고싶으니까 EMPLOOYE가 기준)
SELECT EMP_NAME, DEPT_TITLE
...
SELECT EMP_NAME, DEPT_TITLE
FROM EMPLOYEE, DEPARTMENT
WHERE DEPT_CODE = DEPT_ID(+); -- 생각하는 반대에 + 붙여야함
							  -- 하동운, 이오리는 DEPT_CODE에 있어서 여기다 붙여야 할 것같지만 아님
...
--  조인되는 테이블의 각 행들이 모두 매핑된 데이터가 검색되는 방법(곱집합, 모든 경우의 수 도출)
--> JOIN 구문을 잘못 작성하는 경우 CROSS JOIN의 결과가 조회됨, 조인구문 오류 확인 용도
...
--> 해결 방법 : DEPARTMENT와 LOCATIO 조인 순서를 바꿔서
...
--사원명, 직급명, 급여, 연봉(보너스포함)을 조회하시오.*****못품
--연봉에 보너스포인트를 적용하시오.
SELECT EMP_NAME, JOB_NAME, SALARY*(1 + NVL(BONUS, 0) *12)
FROM EMPLOYEE
NATURAL JOIN JOB, SAL_GRADE
WHERE SAL_LEVEL 
GROUB BY SAL_LEVEL;
...
-- 8. 같은 부서에 근무하는 직원들의
--  사원명, 부서코드, 동료이름을 조회하시오.  *****못품
-- SELF JOIN 사용
SELECT E1.EMP_NAME, DEPT_ID, E2.EMP_NAME
       NVM(E2.EMP_NAME, '없음')
FROM EMPLOYEE E1
JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
LEFT JOIN EMPLOYEE E2 ON(E1.EMP_NAME = E2.EMP NAME);
