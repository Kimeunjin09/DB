/* VIEW
	...
	***** VIEW 사용 시 주의사항 *****
	1) 가상의 테이블(실제 테이블 X) -> ALTER 구문 사용 불가(수정 불가)
...
JOIN JOB USING(JOB_CODE); -- AS 뒤에 서브쿼리 쓴거
-- ORA-01031: 권한이 불충분합니다(kh 계정에 뷰를 만들 권한이 없다는거)
...
-- 생성된 VIEW를 이용해서 조회
SELECT * FROM V_EMP; -- 서브쿼리 실행결과를 VIEW에 담은것!
...
SELECT * FROM V_DCOPY2; -- ROLLBACK확인
