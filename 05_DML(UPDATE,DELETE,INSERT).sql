...
-- 주의 : 혼자서 COMMIT, ROLLBACK 하지 않기 / INSERT, UPDATE, DELETE도 하라고 할때만!


-- 테스트용 테이블 생성(DML 수업 용도)
CREATE TABLE EMPLOYEE2
AS SELECT * FROM EMPLOYEE; -- EMPLOYEE행 조회 결과 값을 다 넣겠다 = 복사본 /  실행!

CREATE TABLE DEPARTMENT2
AS SELECT * FROM DEPARTMENT; -- 실행!

SELECT * FROM EMPLOYEE2; -- 실행
SELECT * FROM DEPARTMENT; -- 실행, SELECT 조회하는건 몇번해도 상관 X
...
VALUES(900, '장채현', '901123-1080503',, '01055569512', 'D1', 'J8', 'S3', 
...
ROLLBACK; -- TCL, INSERT하기 전으로 돌아감

DELETE  FROM EMPLOYEE2
WHERE EMP_ID = 900; -- 데이터 다 지워짐

COMMIT; -- TCL, 하고나면 ROLLBACK을 해도 뒤로 가지 못함, 확실한 데이터에만 사용 

--DML언어로는 계속 롤백 가능하지만, COMMIT, ROLLBACK은 불가

-----------------------------------------------------------------------------------------------

-- 2) INSERT INTO 테이블명(컬럼명, 컬럼명, 컬럼명...) VALUES(데이터1, 데이터2, 데이터3...);
-- 테이블에 내가 선택한 컬럼에 대한 값만 INSERT 할 때 사용
-- 선택 안된 컬럼은 값이 NILL이 들어감 (DEFAULT 존재 시 DEFAULT 값으로 삽입됨)

INSERT INTO EMPLOYEE2(EMP_ID, EMP_NAME, EMP_NO, EMAIL, PHONE, DEPT_CODE, JOB_CODE, SAL_LEVEL, SALARY)
VALUES('900', '장채현', '901123-2345678',  'jang_ch@kh.or.kr', '01055569512', 'D1', 'J7', 'S3', 4300000 );
...
); -- TABLE만들어진거 맞음, 안에 값이없는거(INSERT X)
...
FROM EMPLOYEE2
LEFT JOIN DEPARTMENT2 ON(DEPT_CODE = DEPT_ID);*/ -- 이거를 다른 테이블에 그대로 INSERT 가능 
...
-- [작성법]
-- UPDEATE 테이블명 SET 컬럼명 = 바꿀값
-- [WHERE 컬럼명 비교연산자 비교값];
--> WHERE 조건 중요!
-- UPDATE할 컬럼과 조건을 WHERE절에서 정하기 때문에
...
-- EMPLOYEE2 테이블에서 BOVUS를 받지 않는 사원의
-- BONUS를 0.1로 변경
UPDATE EMPLOYEE2
SET BONUS = 0.1
WHERE BONUS IS NULL;
