...
-- 제약조건명을 지정하지 않을경우 테이블에서 하나씩 찾아야함

-- 2) 삭제 : ALTER TABLE 테이블명
-- 			 DROP CONSTRAINT 제약조건명;

--> 수정은 별도로 존재하지 않기 때문에 삭제 후 추가를 이용해서 수정
...
-- 테이블 Properties의 Constraints에서 확인 가능

-- DEPT_COPY의 DEPT_TITLE 컬럼에 UNIQUE삭제
ALTER TABLE DEPT_COPY
...
-- ORA-00904: : 부적합한 식별자
-- 다른 제약조건이 컬럼을 감시하는 느낌이라면 NOT NULL은 아예 컬럼의 성질을 바꾸는 느낌
...
-- 컬럼 수정
-- [작성법]
-- ALTER TABLE 테이블명 MODIFY 컬럼명 테이터타입;      -> 데이터 타입 변경
...
-- DEPT_ID가 CHAR(2)라서 최대 두글자임 D10은 세글자

-- DEPT_ID 컬럼 수정
ALTER TABLE DEPT_COPY MODIFY DEPT_ID VARCHAR2(3); -- 하고 위에 구문 다시 실행

SELECT * FROM DEPT_COPY;

-- LNAME의 기본값을 'KOREA'로 수정 (DEFAULT 값 변경)
ALTER TABLE DEPT_COPY MODIFY LNAME DEFAULT 'KOREA';
SELECT * FROM DEPT_COPY;
--> 기본값을 변경했다고 해서 기존 데이터가 변하지는 않음 앞으로 들어올 DEFAULT값이 'KOREA'

-- LNAME '한국' -> 'KOREA'로 변경 (기존 데이터 변경)
UPDATE DEPT_COPY
SET LNAME = DEFAULT
WHERE LNAME = '한국';
...
-- 테이블 자체를 드롭시키는게 아닌이상 모든 컬럼 삭제 불가

-- 테이블 삭제
DROP TABLE DEPT_COPY;
SELECT * FROM DEPT_COPY; -- ORA-00942: 테이블 또는 뷰가 존재하지 않습니다 / 삭제 완료
...
-- DEPT_COPY 테이블에 PK추가(컬럼 : DEPT_ID, 제약조건명 : D_COPY_PK)
...
-- [CASCADE CONSTRAINTS]는 삭제옵션

-- 1) 관계가 형성되지 않은 테이블(DCOPY) 삭제
-- 외래키가 설정되지 않은(다른테이블이랑 연결되지 않은)
DROP TABLE DCOPY;
...
INSERT INTO TB2 VALUES(15, 5); -- DML
...
SELECT * FROM TB2; -- 롤백해도 여전히 TB2_COLCOL임
