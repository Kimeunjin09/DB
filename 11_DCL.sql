...
CREATE USER kej_sample IDENTIFIED BY sample1234;

-- 2. 새 연결(접속 방법) 추가
--> ORA-01045: 사용자 KEJ_SAMPLE는 CREATE SESSION 권한을 가지고있지 않음; 로그온이 거절되었습니다
-->  CREATE SESSION 권한(접속 권한)

-- 3. 접속 권한 부여
-- [권한 부여 작성법]
-- GRANT 권한, 권한, ... TO 사용자명;
GRANT CREATE SESSION TO kej_sample;
...
GRANT CREATE TABLE TO kej_sample; -- 생성 권한

ALTER USER kej_sample DEFAULT TABLESPACE
SYSTEM QUOTA UNLIMITED ON SYSTEM; -- 제한 공간없이 할당

-- 7. (kej_sample) 다시 테이블 생성
CREATE TABLE TB_TEST(
	PK_COL NUMBER PRIMARY KEY,
	CONTENT VARCHAR2(100)
);
...
GRANT CONNECT, RESOURCE TO kej_sample;
...
-- kh / kej_sample 사용자 계정끼리 서로 객체 접근 권한 부여

-- 1. (kej_sample) kh계정의 EMPLOYEE 테이블 조회
SELECT * FROM kh.EMPLOYEE;
-- ORA-00942: 테이블 또는 뷰가 존재하지 않습니다
--> 접근권한이 없어서 조회 불가

-- 2.(kh) kej_sample계정에 EMPLOYEE 테이블 조회 권한 부여

-- [객체 권한 부여 방법]
-- GRANT 객체권한 ON 객체명 TO 사용자명
GRANT SELECT ON EMPLOYEE TO kej_sample;

-- 3.(kej_sample) 다시 kh계정의 EMPLOYEE 테이블 조회
SELECT * FROM kh.EMPLOYEE;
...
-- 4. (kej_sample) kh.EMPLOYEE 테이블을 복사한 테이블 생성
...
-- REVOKE 객체권한 ON 객체명 FROM 사용자명
 REVOKE SELECT ON EMPLOYEE FROM kej_sample;

-- 6. (kej_sample) 권한 회수 확인
SELECT * FROM kh.EMPLOYEE;
-- ORA-00942: 테이블 또는 뷰가 존재하지 않습니다
