 ...
 * SELECT DEPT_TITLE, SUM(SALARY)
   ...
   LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
   GROUP BY DEPT_TITLE
   HAVING SUM(SALARY) = 17700000, 800000, 60000, 150000; -- 불가! / IN 사용
...
LEFT JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID); -- 이거 네츄럴조인 밑에하니까 출력값이상한거 확인해보기
...
WHERE EMP_ID NOT IN (SELECT DISTINCT MANAGER_ID
                     FROM EMPLOYEE
                     WHERE MANAGER_ID IS NOT NULL);
-- 정렬을 하려고 할 떄 RESULT SET에 대한 SELECT 구문이 또 필요함                   
...
AND SALARY < ANY (SELECT SALARY
              ...
              NATURAL JOIN JOB
              WHERE JOB_NAME = '과장');
-- > ANY : 과장급여 3개중, 최소금액인 220 보다 더 큰 SALARY찾은거
-- < ANY : 과장급여 3개중, 최대금액인 376 보다 더 작은 SALARY찾은거            
...
JOIN JOB USING(JOB_CODE)
WHERE JOB_NAME = '과장'
AND SALARY > ALL (SELECT SALARY /* MAX(SALARY) 사용가능 */
				  FROM EMPLOYEE
				  JOIN JOB USING(JOB_CODE)
				  WHERE JOB_NAME = '차장');
...
WHERE LOCATION_ID = (SELECT LOCAL_CODE
					 FROM LOCATION
					 WHERE NATIONAL_CODE = 'KO'); -- D,D2,D3,D4,D9
...
WHERE DEPT_CODE = (SELECT DEPT_CODE, JOB_CODE
					...
					WHERE ENT_YN = 'Y'
					AND SUBSTR(EMP_NO, 8, 1) = '2');
-- ORA-00913: 값의 수가 너무 많습니다 --> 비교하려는 행의 갯수가 맞지않음*/
...
WHERE (DEPT_CODE, JOB_CODE) = (SELECT DEPT_CODE, JOB_CODE
					FROM EMPLOYEE
					WHERE ENT_YN = 'Y'
					AND SUBSTR(EMP_NO, 8, 1) = '2'); -- 순서 맞춰야함
...
-- MAIN.JOB_CODE에 J1,J2,J3 차례대로 들어오면서 비교하는 방식
-- MAIN쿼리 -> SUB쿼리 -> MAIN쿼리 순서               
...
WHERE 부서 = '기술지원부';
-- 부서 대신 DEPT_TITLE은 안됨 부서 별칭을 지정해줌으로 DEPT_TITLE이라는 컬럼이 없기때문
...
-- SELECT 해석 순서 때문에 급여 상위 5명이 아니라
...
FROM( SELECT EMP_NAME, SALARY
      FROM EMPLOYEE
      ORDER BY SALARY DESC
); -- 전형돈, 윤은해가 월급이 똑같은데 순위가 다름 19,20
...
EMP_NAME, SALARY
FROM EMPLOYEE; -- 전형돈, 윤은해 둘 다 19 / 20 없어짐
