...
- 인덱스 내부 구조는 B* 트리(이진트리) 형식으로 되어있음.
...
* 인덱스 단점
 - 데이터 변경(INSERT, UPDATE, DELETE) 작업 빈번한 경우 오히려 성능이 저하되는 문제가 발생. (SELECT에서만 사용하는게 좋음)
 - 인덱스도 하나의 객체이다보니 이를 저장하기 위한 별도 공간.
 - 인덱스 생성 시간이 필요 

-- PK랑 UNIQE 제약조건은 자동으로 INDEX 설정
...
WHERE EMP_ID = 215; -- EMP_ID는 PK이기 때문에 INDEX 사용한거 O
